...
//		public String rango, tipo, nombre, valor, renglon, columna;
		...
		for (int i = 0; i < tokenrc.size(); i++) {
			...
			if (tipo[i] == entero || tipo[i] == booleano) {
				...
				if (tipo[i - 1] == publico) {
					if (nombreTipo.equals("Krill"))
						ValoresHaciaTabla("Run", nombreTipo, nombre[i + 1], "0", renglon[i + 1], columna[i + 1]);
					else
						ValoresHaciaTabla("Run", nombreTipo, nombre[i + 1], "false", renglon[i + 1], columna[i + 1]);
