...
public class Semantico {
	...
	public boolean CheckAssignments() {
		for(int i = 0; i < tablaSimbolos.size(); i++ ) {
			...
			if(currentIdentifier.tipo.equals("Krill") && this.isBooleanValue(currentIdentifier.valor)) {
	...
	public boolean checkOperators() {
		...
		String[] lines = code.split("\\r?\\n"); 
		for(int i = 0; i < lines.length; i++) { 
			List<String> expressions = this.getAllIdentifiers(lines[i],"[aA-zZ]{1,40}[0-9]{1,40}[\\s]{0,3}(\\+|-|=|>|<|>=|<=|&|\\||%|!|\\^|\\(|\\=)[\\s]{0,3}[a-z]{1,40}[0-9]{1,40}");
